    ...
    namespace RCM.Backend.Controllers
    {
        ...
        public async Task<IActionResult> CheckIn([FromBody] CheckInRequest request)
        {
            ...
            if (!isOnTime)
            {
                var penalty = new PenaltyPayment
                {
                    ...
                    Reason = $"Đi làm muộn - {shift}",
                    Note = "Chưa thanh toán tiền phạt đi làm muộn",
                    IsDeleted = false
                };

                var salary = await _context.Salaries
                    .FirstOrDefaultAsync(s => s.EmployeeId == request.EmployeeId && s.EndDate == null);
        ...
        [HttpPost("CheckOut")]
            public async Task<IActionResult> CheckOut([FromBody] CheckOutRequest request)
            {
                ...
                if (checkIn.Shift == "Ca sáng")
                {
                    isValidCheckout = currentTime >= morningShiftEnd;
                }
                else // Ca chiều
                {
                    isValidCheckout = currentTime >= afternoonShiftEnd;
                }
